    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/oms/wmsfeighttemplate/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.wmsfeighttemplate.name
                this.dataForm.chargeType = data.wmsfeighttemplate.chargeType
                this.dataForm.firstWeight = data.wmsfeighttemplate.firstWeight
                this.dataForm.firstFee = data.wmsfeighttemplate.firstFee
                this.dataForm.continueWeight = data.wmsfeighttemplate.continueWeight
                this.dataForm.continueFee = data.wmsfeighttemplate.continueFee
                this.dataForm.dest = data.wmsfeighttemplate.dest
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/oms/wmsfeighttemplate/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
